class GeneratePdf < Prawn::Document
    ...
    def table_content
      ...
      table items_rows do
        ...
        self.column_widths = [45, 200, 100]
      end
    end
    
    def items_rows
        [['ID', 'Name','Author Name','Publication Name', 'Quantity','Price']] +
          @items.map do |item|
              [item.id, item.name,item.authorname,item.publicationname, item.quantity,item.price]
        end
    end
end
